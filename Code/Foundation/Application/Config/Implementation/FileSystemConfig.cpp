...
void nsApplicationFileSystemConfig::Apply()
{
  ...
  for (const auto& var : m_DataDirs)
  {
    ...
    {
      nsFileSystem::AddDataDirectory(var.m_sDataDirSpecialPath, "AppFileSystemConfig", var.m_sRootName, (!var.m_sRootName.IsEmpty() && var.m_bWritable) ? nsDataDirUsage::AllowWrites : nsDataDirUsage::ReadOnly).IgnoreResult();
    }
  }
}
