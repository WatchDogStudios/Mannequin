...
{
  NS_ASSERT_DEBUG(code >= 0 && static_cast<nsUInt32>(code) < NS_ARRAY_SIZE(s_szOpCodeNames), "Out of bounds access");
...
nsResult nsExpressionByteCode::Load(nsStreamReader& inout_stream, nsByteArrayPtr externalMemory /*= nsByteArrayPtr()*/)
{
  ...
  else
  {
    ...
    if (nsMemoryUtils::IsAligned(externalMemory.GetPtr(), alignof(nsExpression::StreamDesc)) == false)
    {
      nsLog::Error("External memory is not properly aligned. Expected an alignment of at least {} bytes.", alignof(nsExpression::StreamDesc));
      return NS_FAILURE;
    }
  ...
  // Functions
  {
    pData = nsMemoryUtils::AlignForwards(pData, alignof(nsExpression::FunctionDesc));
  ...
  // ByteCode
  {
    pData = nsMemoryUtils::AlignForwards(pData, alignof(StorageType));
...
void nsExpressionByteCode::Init(nsArrayPtr<const StorageType> byteCode, nsArrayPtr<const nsExpression::StreamDesc> inputs, nsArrayPtr<const nsExpression::StreamDesc> outputs, nsArrayPtr<const nsExpression::FunctionDesc> functions, nsUInt32 uiNumTempRegisters, nsUInt32 uiNumInstructions)
{
  ...
  uiDataSize = nsMemoryUtils::AlignSize<nsUInt32>(uiDataSize, alignof(nsExpression::FunctionDesc));
  uiFunctionsOffset = uiDataSize;
  uiDataSize += functions.ToByteArray().GetCount();

  uiDataSize = nsMemoryUtils::AlignSize<nsUInt32>(uiDataSize, alignof(StorageType));
