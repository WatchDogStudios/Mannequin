...
#if NS_ENABLED(NS_SUPPORTS_IPC)
#  include <PipeChannel_Platform.h>
#endif

static_assert((nsInt32)nsIpcChannel::ConnectionState::Disconnected == (nsInt32)nsIpcChannelEvent::Disconnected);
static_assert((nsInt32)nsIpcChannel::ConnectionState::Connecting == (nsInt32)nsIpcChannelEvent::Connecting);
static_assert((nsInt32)nsIpcChannel::ConnectionState::Connected == (nsInt32)nsIpcChannelEvent::Connected);
...
{
  NS_IGNORE_UNUSED(sAddress);
  NS_IGNORE_UNUSED(mode);
...
#if NS_ENABLED(NS_SUPPORTS_IPC)
  return NS_DEFAULT_NEW(nsPipeChannel_Platform, sAddress, mode);
#else
