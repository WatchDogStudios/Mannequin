...
void nsTelemetry::UpdateNetwork()
{
  ...
  while (true)
  {
    ...
    switch (NetworkEvent.type)
    {
      ...
      {
        if ((nsTelemetry::s_ConnectionMode == nsTelemetry::Server) && (NetworkEvent.peer->eventData != 'EZBC'))
        {
        ...
        else
        {
          ...
          Broadcast(nsTelemetry::Reliable, 'EZBC', 'EZID', &s_uiApplicationID, sizeof(nsUInt32));
...
void nsTelemetry::SendServerName()
{
  ...
  Broadcast(nsTelemetry::Reliable, 'EZBC', 'NAME', data, NS_ARRAY_SIZE(data));
}
...
#else
  NS_IGNORE_UNUSED(tm);
  NS_IGNORE_UNUSED(uiSystemID);
  NS_IGNORE_UNUSED(uiMsgID);
  NS_IGNORE_UNUSED(pData);
  NS_IGNORE_UNUSED(uiDataBytes);
#endif // BUILDSYSTEM_ENABLE_ENET_SUPPORT
}
...
void nsTelemetry::Send(TransmitMode tm, nsUInt32 uiSystemID, nsUInt32 uiMsgID, nsStreamReader& Stream, nsInt32 iDataBytes)
{
  ...
  *((nsUInt32*)&TempData[4]) = uiMsgID;
  ...
  NS_IGNORE_UNUSED(uiSystemID);
  NS_IGNORE_UNUSED(uiMsgID);
  NS_IGNORE_UNUSED(Stream);
  NS_IGNORE_UNUSED(iDataBytes);
#endif // BUILDSYSTEM_ENABLE_ENET_SUPPORT
}
