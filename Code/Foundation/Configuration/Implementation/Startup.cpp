    ...
    while (pSub)
    {
      if (!pSub->m_bStartupDone[stage])
      {
        ...
        while (pSub->GetDependency(iDep) != nullptr)
        {
          ...
          {
            nsLog::Debug("SubSystem '{0}::{1}' could not be started because dependency "
                         "'{2}' is unknown.",
              pSub->GetGroupName(),
          ...
          {
            nsLog::Debug("SubSystem '{0}::{1}' could not be started because "
                                  "dependency '{2}' has not been initialized.",
              pSub->GetGroupName(),
              pSub->GetSubSystemName(), pSub->GetDependency(iDep));
          }
