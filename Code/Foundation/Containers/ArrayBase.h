...
/// \brief Base class for all array containers. Implements all the basic functionality
/// that only requires a pointer and the element count.
template <typename T, typename Derived> class nsArrayBase
{
  public:
    /// \brief Constructor.
    nsArrayBase(); // [tested]
...
#if NS_ENABLED(NS_INTEROP_STL_VECTOR)
    /// \brief Copies the data from a std::vector into this one.
    void operator=(const std::vector<T>& rhs); // [tested]
#endif
    ...
    /// \brief Resizes the array to have exactly uiCount elements. Default constructs
    /// extra elements if the array is grown.
    void SetCount(nsUInt32 uiCount); // [tested]

    /// \brief Resizes the array to have exactly uiCount elements. Constructs all new
    /// elements by copying the FillValue.
    void SetCount(nsUInt32 uiCount, const T& fillValue); // [tested]

    /// \brief Resizes the array to have exactly uiCount elements. Extra elements might be
    /// uninitialized.
    template <typename = void> // Template is used to only conditionally compile this
                               // function in when it is actually used.
                               void SetCountUninitialized(nsUInt32 uiCount); // [tested]

    /// \brief Ensures the container has at least \a uiCount elements. Ie. calls
    /// SetCount() if the container has fewer elements, does nothing otherwise.
    ...
    /// \brief Inserts all elements in the range starting at the given index, shifting the
    /// elements after the index.
    void InsertRange(const nsArrayPtr<const T>& range, nsUInt32 uiIndex); // [tested]

    /// \brief Removes the first occurrence of value and fills the gap by shifting all
    /// following elements
    bool RemoveAndCopy(const T& value); // [tested]

    /// \brief Removes the first occurrence of value and fills the gap by swapping in the
    /// last element
    bool RemoveAndSwap(const T& value); // [tested]

    /// \brief Removes the element at index and fills the gap by shifting all following
    /// elements
    void RemoveAtAndCopy(nsUInt32 uiIndex, nsUInt32 uiNumElements = 1); // [tested]

    /// \brief Removes the element at index and fills the gap by swapping in the last
    /// element
    void RemoveAtAndSwap(nsUInt32 uiIndex, nsUInt32 uiNumElements = 1); // [tested]

    /// \brief Searches for the first occurrence of the given value and returns its index
    /// or nsInvalidIndex if not found.
    nsUInt32 IndexOf(const T& value, nsUInt32 uiStartIndex = 0) const; // [tested]

    /// \brief Searches for the last occurrence of the given value and returns its index
    /// or nsInvalidIndex if not found.
    nsUInt32 LastIndexOf(
      const T& value, nsUInt32 uiStartIndex = nsInvalidIndex) const; // [tested]

    /// \brief Grows the array by one element and returns a reference to the newly created
    /// element.
    T& ExpandAndGetRef(); // [tested]

    /// \brief Expands the array by N new items and returns a pointer to the first new
    /// one.
    ...
    /// \brief Pushes all elements in range at the end of the array. Increases the
    ...
    template <typename Comparer> void Sort(const Comparer& comparer); // [tested]
    ...
    /// \brief Returns an array pointer to the array data, or an empty array pointer if
    /// the array is empty.
    ...
    /// \brief Returns a byte array pointer to the array data, or an empty array pointer
    /// if the array is empty.
    nsArrayPtr<typename nsArrayPtr<T>::ByteType> GetByteArrayPtr(); // [tested]

    /// \brief Returns a byte array pointer to the array data, or an empty array pointer
    /// if the array is empty.
    nsArrayPtr<typename nsArrayPtr<const T>::ByteType>
    GetByteArrayPtr() const; // [tested]

    /// \brief Returns the reserved number of elements that the array can hold without
    /// reallocating.
  ...
  protected:
    ...
    /// \brief The number of elements which can be stored in the array without
    /// re-allocating.
...
template <typename T, typename Derived>
typename nsArrayBase<T, Derived>::const_iterator begin(
  const nsArrayBase<T, Derived>& container)
{
    return container.GetData();
}

template <typename T, typename Derived>
typename nsArrayBase<T, Derived>::const_iterator cbegin(
  const nsArrayBase<T, Derived>& container)
{
    return container.GetData();
}
...
template <typename T, typename Derived>
typename nsArrayBase<T, Derived>::reverse_iterator rbegin(
  nsArrayBase<T, Derived>& ref_container)
{
    return typename nsArrayBase<T, Derived>::reverse_iterator(
      ref_container.GetData() + ref_container.GetCount() - 1);
}

template <typename T, typename Derived>
typename nsArrayBase<T, Derived>::const_reverse_iterator rbegin(
  const nsArrayBase<T, Derived>& container)
{
    return typename nsArrayBase<T, Derived>::const_reverse_iterator(
      container.GetData() + container.GetCount() - 1);
}

template <typename T, typename Derived>
typename nsArrayBase<T, Derived>::const_reverse_iterator crbegin(
  const nsArrayBase<T, Derived>& container)
{
    return typename nsArrayBase<T, Derived>::const_reverse_iterator(
      container.GetData() + container.GetCount() - 1);
...
template <typename T, typename Derived>
typename nsArrayBase<T, Derived>::const_iterator end(
  const nsArrayBase<T, Derived>& container)
{
...
template <typename T, typename Derived>
typename nsArrayBase<T, Derived>::const_iterator cend(
  const nsArrayBase<T, Derived>& container)
{
    return container.GetData() + container.GetCount();
}
...
template <typename T, typename Derived>
typename nsArrayBase<T, Derived>::reverse_iterator rend(
  nsArrayBase<T, Derived>& ref_container)
{
    return
      typename nsArrayBase<T, Derived>::reverse_iterator(ref_container.GetData() - 1);
}

template <typename T, typename Derived>
typename nsArrayBase<T, Derived>::const_reverse_iterator rend(
  const nsArrayBase<T, Derived>& container)
{
    return
      typename nsArrayBase<T, Derived>::const_reverse_iterator(container.GetData() - 1);
}

template <typename T, typename Derived>
typename nsArrayBase<T, Derived>::const_reverse_iterator crend(
  const nsArrayBase<T, Derived>& container)
{
    return
      typename nsArrayBase<T, Derived>::const_reverse_iterator(container.GetData() - 1);
