template <typename T, typename Derived> nsArrayBase<T, Derived>::nsArrayBase() = default;

template <typename T, typename Derived> nsArrayBase<T, Derived>::~nsArrayBase()
{
    NS_ASSERT_DEBUG(m_uiCount == 0,
...
void nsArrayBase<T, Derived>::operator=(const nsArrayPtr<const T>& rhs)
{
    if (this->GetData() == rhs.GetPtr())
    {
        ...
        NS_ASSERT_DEV(m_uiCount > rhs.GetCount(),
    ...
    if (uiNewCount > uiOldCount)
    {
        ...
        nsMemoryUtils::CopyConstructArray(
          pElements + uiOldCount, rhs.GetPtr() + uiOldCount, uiNewCount - uiOldCount);
    }
    else
    {
        T* pElements = static_cast<Derived*>(this)->GetElementsPtr();
...
// NOTE(Mikael A): Same as above, but for std::vector
template <typename T, typename Derived>
void nsArrayBase<T, Derived>::operator=(const std::vector<T>& rhs)
{
    const nsUInt32 uiOldCount = m_uiCount;
    const nsUInt32 uiNewCount = static_cast<nsUInt32>(rhs.size());
    if (uiNewCount > uiOldCount)
    {
        static_cast<Derived*>(this)->Reserve(uiNewCount);
        T* pElements = static_cast<Derived*>(this)->GetElementsPtr();
        nsMemoryUtils::Copy(pElements, rhs.data(), uiOldCount);
        nsMemoryUtils::CopyConstructArray(
          pElements + uiOldCount, rhs.data() + uiOldCount, uiNewCount - uiOldCount);
    }
    else
    {
        T* pElements = static_cast<Derived*>(this)->GetElementsPtr();
        nsMemoryUtils::Copy(pElements, rhs.data(), uiNewCount);
        nsMemoryUtils::Destruct(pElements + uiNewCount, uiOldCount - uiNewCount);
    }
    m_uiCount = uiNewCount;
}
...
{
    return nsArrayPtr<const T>(
      static_cast<const Derived*>(this)->GetElementsPtr(), m_uiCount);
}
...
bool nsArrayBase<T, Derived>::operator==(const nsArrayBase<T, Derived>& rhs) const
{
    ...
    return nsMemoryUtils::IsEqual(
      static_cast<const Derived*>(this)->GetElementsPtr(), rhs.GetData(), m_uiCount);
}

template <typename T, typename Derived>
NS_ALWAYS_INLINE bool nsArrayBase<T, Derived>::operator<(
  const nsArrayBase<T, Derived>& rhs) const
{
...
bool nsArrayBase<T, Derived>::operator==(const nsArrayPtr<const T>& rhs) const
{
    ...
    return nsMemoryUtils::IsEqual(
      static_cast<const Derived*>(this)->GetElementsPtr(), rhs.GetPtr(), m_uiCount);
}
#endif

template <typename T, typename Derived>
NS_ALWAYS_INLINE bool nsArrayBase<T, Derived>::operator<(
  const nsArrayPtr<const T>& rhs) const
{
    return GetArrayPtr() < rhs;
}

template <typename T, typename Derived>
NS_ALWAYS_INLINE const T& nsArrayBase<T, Derived>::operator[](
  const nsUInt32 uiIndex) const
{
    NS_ASSERT_DEBUG(uiIndex < m_uiCount,
      "Out of bounds access. Array has {0} elements, trying to access element at index "
      "{1}.",
      m_uiCount, uiIndex);
...
void nsArrayBase<T, Derived>::SetCount(nsUInt32 uiCount)
{
    ...
    {
        static_cast<Derived*>(this)->Reserve(uiNewCount);
        nsMemoryUtils::Construct<ConstructAll>(
          static_cast<Derived*>(this)->GetElementsPtr() + uiOldCount,
          uiNewCount - uiOldCount);
    }
    else if (uiNewCount < uiOldCount)
    {
        nsMemoryUtils::Destruct(
          static_cast<Derived*>(this)->GetElementsPtr() + uiNewCount,
          uiOldCount - uiNewCount);
    }
...
void nsArrayBase<T, Derived>::SetCount(nsUInt32 uiCount, const T& fillValue)
{
    ...
    {
        static_cast<Derived*>(this)->Reserve(uiNewCount);
        nsMemoryUtils::CopyConstruct(
          static_cast<Derived*>(this)->GetElementsPtr() + uiOldCount, fillValue,
          uiNewCount - uiOldCount);
    }
    else if (uiNewCount < uiOldCount)
    {
...
template <typename T, typename Derived>
template <typename> // Second template needed so that the compiler does only instantiate
                    // it when called. Otherwise the static_assert would trigger
                    // early.
                    void nsArrayBase<T, Derived>::SetCountUninitialized(nsUInt32 uiCount)
{
    static_assert(nsIsPodType<T>::value == nsTypeIsPod::value,
...
template <typename T, typename Derived> void nsArrayBase<T, Derived>::Clear()
{
...
{
    NS_ASSERT_DEV(uiIndex <= m_uiCount,
      "Invalid index. Array has {0} elements, trying to insert element at index {1}.",
      m_uiCount, uiIndex);

    static_cast<Derived*>(this)->Reserve(m_uiCount + 1);

    nsMemoryUtils::Prepend(static_cast<Derived*>(this)->GetElementsPtr() + uiIndex, value,
      m_uiCount - uiIndex);
    m_uiCount++;
}
...
void nsArrayBase<T, Derived>::InsertAt(nsUInt32 uiIndex, T&& value)
{
    ...
    nsMemoryUtils::Prepend(static_cast<Derived*>(this)->GetElementsPtr() + uiIndex,
      std::move(value), m_uiCount - uiIndex);
    m_uiCount++;
}

template <typename T, typename Derived>
void nsArrayBase<T, Derived>::InsertRange(
  const nsArrayPtr<const T>& range, nsUInt32 uiIndex)
{
    const nsUInt32 uiRangeCount = range.GetCount();
    static_cast<Derived*>(this)->Reserve(m_uiCount + uiRangeCount);

    nsMemoryUtils::Prepend(static_cast<Derived*>(this)->GetElementsPtr() + uiIndex,
      range.GetPtr(), uiRangeCount, m_uiCount - uiIndex);
...
template <typename T, typename Derived>
void nsArrayBase<T, Derived>::RemoveAtAndCopy(
  nsUInt32 uiIndex, nsUInt32 uiNumElements /*= 1*/)
{
    NS_ASSERT_DEV(uiIndex + uiNumElements <= m_uiCount,
      "Out of bounds access. Array has {0} elements, trying to remove element at index "
      "{1}.",
    ...
    nsMemoryUtils::RelocateOverlapped(
      pElements + uiIndex, pElements + uiIndex + uiNumElements, m_uiCount - uiIndex);
}

template <typename T, typename Derived>
void nsArrayBase<T, Derived>::RemoveAtAndSwap(
  nsUInt32 uiIndex, nsUInt32 uiNumElements /*= 1*/)
{
    NS_ASSERT_DEV(uiIndex + uiNumElements <= m_uiCount,
      "Out of bounds access. Array has {0} elements, trying to remove element at index "
...
template <typename T, typename Derived> T& nsArrayBase<T, Derived>::ExpandAndGetRef()
{
    static_cast<Derived*>(this)->Reserve(m_uiCount + 1);
...
void nsArrayBase<T, Derived>::PushBack(const T& value)
{
    ...
    nsMemoryUtils::CopyConstruct(
      static_cast<Derived*>(this)->GetElementsPtr() + m_uiCount, value, 1);
    m_uiCount++;
}

template <typename T, typename Derived> void nsArrayBase<T, Derived>::PushBack(T&& value)
{
    static_cast<Derived*>(this)->Reserve(m_uiCount + 1);

    nsMemoryUtils::MoveConstruct<T>(
      static_cast<Derived*>(this)->GetElementsPtr() + m_uiCount, std::move(value));
    m_uiCount++;
}
...
{
    NS_ASSERT_DEBUG(m_uiCount < m_uiCapacity,
...
void nsArrayBase<T, Derived>::PushBackRange(const nsArrayPtr<const T>& range)
{
    ...
    nsMemoryUtils::CopyConstructArray(
      static_cast<Derived*>(this)->GetElementsPtr() + m_uiCount, range.GetPtr(),
      uiRangeCount);
    m_uiCount += uiRangeCount;
}
...
{
    NS_ASSERT_DEV(m_uiCount >= uiCountToRemove,
      "Out of bounds access. Array has {0} elements, trying to pop {1} elements.",
      m_uiCount, uiCountToRemove);

    m_uiCount -= uiCountToRemove;
    nsMemoryUtils::Destruct(
      static_cast<Derived*>(this)->GetElementsPtr() + m_uiCount, uiCountToRemove);
}
...
{
    NS_ASSERT_DEBUG(
      m_uiCount > 0, "Out of bounds access. Trying to peek into an empty array.");
...
template <typename T, typename Derived> void nsArrayBase<T, Derived>::Sort()
{
    if (m_uiCount > 1)
    {
        nsArrayPtr<T> ar = *this;
...
template <typename T, typename Derived>
NS_ALWAYS_INLINE nsArrayPtr<typename nsArrayPtr<T>::ByteType>
nsArrayBase<T, Derived>::GetByteArrayPtr()
{
    return GetArrayPtr().ToByteArray();
}

template <typename T, typename Derived>
NS_ALWAYS_INLINE nsArrayPtr<typename nsArrayPtr<const T>::ByteType>
nsArrayBase<T, Derived>::GetByteArrayPtr() const
{
    return GetArrayPtr().ToByteArray();
}
