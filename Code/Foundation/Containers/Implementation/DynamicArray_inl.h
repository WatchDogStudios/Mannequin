...
template <typename T>
inline std::vector<T> nsDynamicArrayBase<T>::ToStdVector() const
{
    return std::vector<T>(GetElementsPtr(), GetElementsPtr() + this->m_uiCount);
}
#endif
...
template <typename T, typename AllocatorWrapper /*= nsDefaultAllocatorWrapper*/>
std::vector<T> nsDynamicArray<T, AllocatorWrapper>::ToStdVector() const
{
    return nsDynamicArrayBase<T>::ToStdVector();
}
#endif
