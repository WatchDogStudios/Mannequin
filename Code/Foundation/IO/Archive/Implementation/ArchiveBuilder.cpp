...
#else
  NS_IGNORE_UNUSED(sAbsFolderPath);
  NS_IGNORE_UNUSED(defaultMode);
  NS_IGNORE_UNUSED(callback);
...
{
  NS_IGNORE_UNUSED(uiCurEntry);
  NS_IGNORE_UNUSED(uiMaxEntries);
  NS_IGNORE_UNUSED(sSourceFile);
  return true;
}

bool nsArchiveBuilder::WriteFileProgressCallback(nsUInt64 bytesWritten, nsUInt64 bytesTotal) const
{
  NS_IGNORE_UNUSED(bytesWritten);
  NS_IGNORE_UNUSED(bytesTotal);
  return true;
}
