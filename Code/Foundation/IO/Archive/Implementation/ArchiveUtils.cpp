...
nsResult nsArchiveUtils::ReadHeader(nsStreamReader& inout_stream, nsUInt8& out_uiVersion)
{
  ...
  char szTag[10];
  if (inout_stream.ReadBytes(szTag, 10) != 10 || !nsStringUtils::IsEqual(szTag, "EZARCHIVE"))
  {
...
{
  NS_IGNORE_UNUSED(iCompressionLevel);
  ...
  while (true)
  {
    ...
    NS_SUCCEED_OR_RETURN(pWriter->WriteBytes(buf.GetData(), uiRead));
  }
...
static nsResult VerifyEndMarker(nsUInt64 uiArchiveDataSize, const void* pArchiveDataBuffer, nsUInt8 uiArchiveVersion)
{
  ...
  const void* pStart = nsMemoryUtils::AddByteOffset(pArchiveDataBuffer, static_cast<ptrdiff_t>(uiArchiveDataSize - uiEndMarkerSize));
...
nsResult nsArchiveUtils::ExtractTOCMeta(nsUInt64 uiArchiveEndingDataSize, const void* pArchiveEndingDataBuffer, TOCMeta& ref_tocMeta, nsUInt8 uiArchiveVersion)
{
  ...
  // read the TOC meta data
  {
    ...
    const void* pTocMetaStart = nsMemoryUtils::AddByteOffset(pArchiveEndingDataBuffer, static_cast<ptrdiff_t>(uiArchiveEndingDataSize - uiEndMarkerSize - uiTocMetaSize));
...
nsResult nsArchiveUtils::ExtractTOC(nsUInt64 uiArchiveEndingDataSize, const void* pArchiveEndingDataBuffer, nsArchiveTOC& ref_toc, nsUInt8 uiArchiveVersion)
{
  ...
  // get toc data ptr
  const void* pTocStart = nsMemoryUtils::AddByteOffset(pArchiveEndingDataBuffer, static_cast<ptrdiff_t>(uiArchiveEndingDataSize - tocMeta.m_uiTocOffsetFromArchiveEnd));
