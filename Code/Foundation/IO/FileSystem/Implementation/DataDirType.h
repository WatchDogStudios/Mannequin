...
struct nsFileStats;
class nsDataDirectoryType;

/// \brief Describes in which mode a data directory is mounted.
enum class nsDataDirUsage
{
  ReadOnly,
  AllowWrites,
};

struct nsDataDirectoryInfo
{
  nsDataDirUsage m_Usage;

  nsString m_sRootName;
  nsString m_sGroup;
  nsDataDirectoryType* m_pDataDirType = nullptr;
};
...
protected:
  ...
  virtual nsDataDirectoryWriter* OpenFileToWrite(nsStringView sFile, nsFileShareMode::Enum FileShareMode)
  {
    NS_IGNORE_UNUSED(sFile);
  ...
  virtual void DeleteFile(nsStringView sFile) { NS_IGNORE_UNUSED(sFile); }
  ...
  virtual bool ResolveAssetRedirection(nsStringView sPathOrAssetGuid, nsStringBuilder& out_sRedirection)
  {
...
protected:
  ...
  virtual void OnReaderWriterClose(nsDataDirectoryReaderWriterBase* pClosed) { NS_IGNORE_UNUSED(pClosed); }
...
protected:
  ...
  nsDataDirectoryType* m_pDataDirType;
