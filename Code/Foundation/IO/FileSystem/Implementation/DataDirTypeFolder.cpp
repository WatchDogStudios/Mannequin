...
namespace nsDataDirectory
{
  ...
  nsDataDirectoryType* FolderType::Factory(nsStringView sDataDirectory, nsStringView sGroup, nsStringView sRootName, nsDataDirUsage usage)
  {
    NS_IGNORE_UNUSED(sGroup);
    NS_IGNORE_UNUSED(sRootName);
    NS_IGNORE_UNUSED(usage);
  ...
  void FolderType::LoadRedirectionFile()
  {
    ...
    if (!s_sRedirectionFile.IsEmpty())
    {
      ...
      if (file.Open(sRedirectionFile, nsFileOpenMode::Read).Succeeded())
      {
        ...
        content.Reserve((nsUInt32)(file.GetFileSize() + 1));
        char uiTemp[4096];
  ...
  {
    NS_IGNORE_UNUSED(bOneSpecificDataDir);
    ...
    return sPath.IsAbsolutePath() && nsOSFile::ExistsFile(sPath);
  }
  ...
  nsResult FolderType::InternalInitializeDataDirectory(nsStringView sDirectory)
  {
    ...
    if (!m_sRedirectedDataDirPath.IsAbsolutePath() || !nsOSFile::ExistsDirectory(m_sRedirectedDataDirPath))
      return NS_FAILURE;
  ...
  {
    NS_IGNORE_UNUSED(bSpecificallyThisDataDir);
