...
{
  // This assert helps finding cases where the nsFileSystem is used without ns being properly initialized or already shutdown.
...
nsResult nsFileSystem::AddDataDirectory(nsStringView sDataDirectory, nsStringView sGroup, nsStringView sRootName, nsDataDirUsage usage)
{
  NS_ASSERT_DEV(s_pData != nullptr, "FileSystem is not initialized.");
  NS_ASSERT_DEV(usage != nsDataDirUsage::AllowWrites || !sRootName.IsEmpty(), "A data directory must have a non-empty, unique name to be mounted for write access");
  ...
  if (!failed)
  {
    ...
    for (nsInt32 i = s_pData->m_DataDirFactories.GetCount() - 1; i >= 0; --i)
    {
      ...
      if (pDataDir != nullptr)
      {
        ...
        nsLog::Dev("Added Data Directory '{}' -> '{}'", sRootName, sDataDirectory);
        return NS_SUCCESS;
      }
    }
  }
...
const nsDataDirectoryInfo* nsFileSystem::FindDataDirectoryWithRoot(nsStringView sRootName)
{
...
const nsDataDirectoryInfo& nsFileSystem::GetDataDirectoryInfo(nsUInt32 uiDataDirIndex)
{
  NS_ASSERT_DEV(s_pData != nullptr, "FileSystem is not initialized.");
...
nsStringView nsFileSystem::GetDataDirRelativePath(nsStringView sPath, nsUInt32 uiDataDir)
{
  ...
  const nsString128& sRedDirPath = s_pData->m_DataDirectories[uiDataDir].m_pDataDirType->GetRedirectedDataDirectoryPath();
  ...
  const nsString128& sDirPath = s_pData->m_DataDirectories[uiDataDir].m_pDataDirType->GetDataDirectoryPath();
...
nsDataDirectoryInfo* nsFileSystem::GetDataDirForRoot(const nsString& sRoot)
{
  NS_ASSERT_DEV(s_pData != nullptr, "FileSystem is not initialized.");
  NS_LOCK(s_pData->m_FsMutex);
...
void nsFileSystem::DeleteFile(nsStringView sFile)
{
  ...
  for (nsInt32 i = (nsInt32)s_pData->m_DataDirectories.GetCount() - 1; i >= 0; --i)
  {
    ...
    if (s_pData->m_DataDirectories[i].m_Usage != nsDataDirUsage::AllowWrites)
      continue;
...
nsDataDirectoryReader* nsFileSystem::GetFileReader(nsStringView sFile, nsFileShareMode::Enum FileShareMode, bool bAllowFileEvents)
{
  ...
  for (nsInt32 i = (nsInt32)s_pData->m_DataDirectories.GetCount() - 1; i >= 0; --i)
  {
    ...
    nsDataDirectoryReader* pReader = s_pData->m_DataDirectories[i].m_pDataDirType->OpenFileToRead(sRelPath, FileShareMode, bOneSpecificDataDir);
...
nsDataDirectoryWriter* nsFileSystem::GetFileWriter(nsStringView sFile, nsFileShareMode::Enum FileShareMode, bool bAllowFileEvents)
{
  ...
  for (nsInt32 i = (nsInt32)s_pData->m_DataDirectories.GetCount() - 1; i >= 0; --i)
  {
    ...
    nsDataDirectoryWriter* pWriter = s_pData->m_DataDirectories[i].m_pDataDirType->OpenFileToWrite(sRelPath, FileShareMode);
...
nsResult nsFileSystem::ResolvePath(nsStringView sPath, nsStringBuilder* out_pAbsolutePath, nsStringBuilder* out_pDataDirRelativePath, const nsDataDirectoryInfo** out_pDataDir /*= nullptr*/)
{
  NS_ASSERT_DEV(s_pData != nullptr, "FileSystem is not initialized.");
  ...
  if (sPath.StartsWith(":"))
  {
    ...
    const nsDataDirectoryInfo* pDataDir = GetDataDirForRoot(sRootName);
  ...
  else if (nsPathUtils::IsAbsolutePath(sPath))
  {
    ...
    for (nsUInt32 dd = s_pData->m_DataDirectories.GetCount(); dd > 0; --dd)
    {
      ...
      if (nsPathUtils::IsSubPath(dir.m_pDataDirType->GetRedirectedDataDirectoryPath(), absPath))
      {
...
{
  NS_IGNORE_UNUSED(sExpectedSubFolder);
...
#if NS_ENABLED(NS_SUPPORTS_UNRESTRICTED_FILE_ACCESS)
...
void nsFileSystem::SetSpecialDirectory(nsStringView sName, nsStringView sReplacement)
{
  ...
  else
  {
    ...
    nsLog::Dev("Setting special directory '{}' to '{}'", sName, sReplacement);
  }
}
...
void nsFileSystem::StartSearch(nsFileSystemIterator& ref_iterator, nsStringView sSearchTerm, nsBitflags<nsFileSystemIteratorFlags> flags /*= nsFileSystemIteratorFlags::Default*/)
{
  ...
  if (sSearchTerm.IsRootedPath())
  {
    ...
    const nsDataDirectoryInfo* pDataDir = FindDataDirectoryWithRoot(root);
    if (pDataDir == nullptr)
      return;
