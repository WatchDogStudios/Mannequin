...
public:
  ...
  nsResult GetType(nsStringView sPath, Type& out_type);
...
template <typename T>
nsResult nsFileSystemMirror<T>::GetType(nsStringView sPath0, Type& out_type)
{
  nsStringBuilder sPath = sPath0;
  DirEntry* dir = FindDirectory(sPath);
  if (dir == nullptr)
  ...
  if (it.IsValid())
  {
    out_type = nsFileSystemMirror::Type::File;
    return NS_SUCCESS;
  }
  ...
  if (itDir.IsValid())
  {
    out_type = nsFileSystemMirror::Type::Directory;
    return NS_SUCCESS;
  }
