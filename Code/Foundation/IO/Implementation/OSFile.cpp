...
#if NS_ENABLED(NS_COMPILE_FOR_DEVELOPMENT)
  const nsTime tdiff = nsTime::Now() - t0;
#else
  const nsTime tdiff = nsTime::MakeZero();
#endif
...
bool nsOSFile::ExistsFile(nsStringView sFile)
{
  ...
  NS_ASSERT_DEV(s.IsAbsolutePath(), "Path must be absolute: '{}'", sFile);
...
bool nsOSFile::ExistsDirectory(nsStringView sDirectory)
{
  ...
  NS_ASSERT_DEV(s.IsAbsolutePath(), "Path must be absolute: '{}'", sDirectory);
