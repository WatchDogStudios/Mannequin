...
#define NS_IMPLEMENT_SERIALIZATION_CONTEXT(type)                                                                                        \
  thread_local type* NS_PP_CONCAT(s_pActiveContext, type);                                                                              \
  type* type::GetContext()                                                                                                              \
  {                                                                                                                                     \
    return NS_PP_CONCAT(s_pActiveContext, type);                                                                                        \
  }                                                                                                                                     \
  void type::SetContext(nsSerializationContext* pContext)                                                                               \
  {                                                                                                                                     \
    NS_ASSERT_DEV(pContext == nullptr || NS_PP_CONCAT(s_pActiveContext, type) == nullptr, "Only one context can be active at a time."); \
    NS_PP_CONCAT(s_pActiveContext, type) = static_cast<type*>(pContext);                                                                \
  }
