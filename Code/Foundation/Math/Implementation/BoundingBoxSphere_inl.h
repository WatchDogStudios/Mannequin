...
NS_FORCE_INLINE nsBoundingBoxSphereTemplate<Type> nsBoundingBoxSphereTemplate<Type>::MakeInvalid()
{
  ...
  res.m_vBoxHalfExtents.Set(-nsMath::MaxValue<Type>());
  return res;
}
...
nsBoundingBoxSphereTemplate<Type> nsBoundingBoxSphereTemplate<Type>::MakeFromBoxAndSphere(const nsBoundingBoxTemplate<Type>& box, const nsBoundingSphereTemplate<Type>& sphere)
{
  ...
  res.m_vCenter = box.GetCenter();
  res.m_vBoxHalfExtents = box.GetHalfExtents();
  res.m_fSphereRadius = nsMath::Min(res.m_vBoxHalfExtents.GetLength(), (sphere.m_vCenter - res.m_vCenter).GetLength() + sphere.m_fRadius);
  return res;
}
...
{
  return (m_vCenter.IsNaN() || nsMath::IsNaN(m_fSphereRadius) || m_vBoxHalfExtents.IsNaN());
}

template <typename Type>
NS_FORCE_INLINE const nsBoundingBoxTemplate<Type> nsBoundingBoxSphereTemplate<Type>::GetBox() const
{
  return nsBoundingBoxTemplate<Type>::MakeFromMinMax(m_vCenter - m_vBoxHalfExtents, m_vCenter + m_vBoxHalfExtents);
}
...
void nsBoundingBoxSphereTemplate<Type>::Transform(const nsMat4Template<Type>& mTransform)
{
  ...
  m_vBoxHalfExtents = mAbsRotation.TransformDirection(m_vBoxHalfExtents).CompMin(nsVec3(m_fSphereRadius));
}
