...
bool nsIntersectionUtils::RayTriangleIntersection(const nsVec3& vRayStartPos, const nsVec3& vRayDir, const nsVec3& vVertex0, const nsVec3& vVertex1, const nsVec3& vVertex2, float* out_pIntersectionTime /*= nullptr*/, nsVec3* out_pIntersectionPoint /*= nullptr*/)
{
  const nsPlane plane = nsPlane::MakeFromPoints(vVertex0, vVertex1, vVertex2);
  ...
  {
    const nsVec3 edge = vVertex1 - vVertex0;
    const nsVec3 vp = vIntersection - vVertex0;
  ...
  {
    const nsVec3 edge = vVertex2 - vVertex1;
    const nsVec3 vp = vIntersection - vVertex1;
    if (plane.m_vNormal.Dot(edge.CrossRH(vp)) < 0)
    {
      return false;
    }
  ...
  {
    const nsVec3 edge = vVertex0 - vVertex2;
    const nsVec3 vp = vIntersection - vVertex2;
    if (plane.m_vNormal.Dot(edge.CrossRH(vp)) < 0)
    {
      return false;
    }
  ...
  nsUInt32 uiNumVertices, float* out_pIntersectionTime, nsVec3* out_pIntersectionPoint, nsUInt32 uiVertexStride)
{
  ...
  nsPlane plane = nsPlane::MakeFromPoints(*pPolygonVertices, *nsMemoryUtils::AddByteOffset(pPolygonVertices, uiVertexStride), *nsMemoryUtils::AddByteOffset(pPolygonVertices, uiVertexStride * 2));

  NS_ASSERT_DEBUG(plane.IsValid(), "The given polygon's plane is invalid (computed from the first three vertices only).");
  ...
  for (nsUInt32 i = 0; i < uiNumVertices; ++i)
  {
    ...
    const nsVec3 edge = vThisPoint - vPrevPoint;
    const nsVec3 vp = vIntersection - vPrevPoint;
    if (plane.m_vNormal.Dot(edge.CrossRH(vp)) < 0)
    {
      return false;
    }
