...
#if __castxml__
    return 0;
#elif NS_ENABLED(NS_COMPILER_MSVC)
    unsigned long uiIndex = 0;
...
#  endif
  ...
  template <typename Type>
  NS_ALWAYS_INLINE constexpr Type Bitmask_LowN(nsUInt32 uiNumBitsToSet)
  {
    return (uiNumBitsToSet >= sizeof(Type) * 8) ? ~static_cast<Type>(0) : ((static_cast<Type>(1) << uiNumBitsToSet) - static_cast<Type>(1));
  }

  template <typename Type>
  NS_ALWAYS_INLINE constexpr Type Bitmask_HighN(nsUInt32 uiNumBitsToSet)
  {
  ...
  template <nsUInt32 NumBits>
  inline nsUInt32 ColorFloatToUnsignedInt(float value)
  {
    constexpr float fMaxValue = static_cast<float>(nsMath::Bitmask_LowN<nsUInt32>(NumBits));
    ...
    {
      return static_cast<nsUInt32>(Saturate(value) * fMaxValue + 0.5f);
    }
  }

  NS_ALWAYS_INLINE nsUInt8 ColorFloatToByte(float value)
  {
    return static_cast<nsUInt8>(ColorFloatToUnsignedInt<8>(value));
  }

  NS_ALWAYS_INLINE nsUInt16 ColorFloatToShort(float value)
  {
    return static_cast<nsUInt16>(ColorFloatToUnsignedInt<16>(value));
  }
...
NS_ALWAYS_INLINE constexpr nsUInt32 nsMath::WrapUInt(nsUInt32 uiValue, nsUInt32 uiExcludedMaxValue)
{
  return uiValue % uiExcludedMaxValue;
}

NS_ALWAYS_INLINE constexpr nsInt32 nsMath::WrapInt(nsInt32 iValue, nsUInt32 uiExcludedMaxValue)
{
  const nsInt32 wrapped = (iValue % static_cast<nsInt32>(uiExcludedMaxValue));
  return wrapped >= 0 ? wrapped : (wrapped + uiExcludedMaxValue);
}

NS_ALWAYS_INLINE constexpr nsInt32 nsMath::WrapInt(nsInt32 iValue, nsInt32 iMinValue, nsInt32 iExcludedMaxValue)
{
  NS_ASSERT_DEBUG(iMinValue < iExcludedMaxValue, "Invalid range to wrap integer around.");
  return iMinValue + WrapInt(iValue - iMinValue, static_cast<nsUInt32>(iExcludedMaxValue - iMinValue));
}

NS_ALWAYS_INLINE float nsMath::WrapFloat01(float fValue)
{
  if (fValue < 0.0f)
  {
...
NS_ALWAYS_INLINE float nsMath::WrapFloat(float fValue, float fMinValue, float fMaxValue)
{
  const float range = fMaxValue - fMinValue;
  return fMinValue + WrapFloat01((fValue - fMinValue) / range) * range;
}

NS_ALWAYS_INLINE constexpr nsUInt64 nsMath::MakeUInt64(nsUInt32 uiHigh32, nsUInt32 uiLow32)
{
  return (static_cast<nsUInt64>(uiHigh32) << 32) | static_cast<nsUInt64>(uiLow32);
}
