...
{
  if (GetVectorPart().IsEqual(qOther.GetVectorPart(), fEpsilon) && nsMath::IsEqual(w, qOther.w, fEpsilon))
  {
    return true;
  }
  ...
  if ((A1.IsEqualSimple(A2, nsAngle::MakeFromDegree(static_cast<float>(360.0f * fEpsilon)))) && (vA1.IsEqual(vA2, fEpsilon)))
    return true;
