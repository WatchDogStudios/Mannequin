...
namespace nsMath
{
  ...
  [[nodiscard]] constexpr nsUInt32 WrapUInt(nsUInt32 uiValue, nsUInt32 uiExcludedMaxValue); // [tested]
  ...
  /// A value of 0 for uiExcludedMaxValue is invalid and results in a division by zero error.
  [[nodiscard]] constexpr nsInt32 WrapInt(nsInt32 iValue, nsUInt32 uiExcludedMaxValue); // [tested]
  ...
  [[nodiscard]] constexpr nsInt32 WrapInt(nsInt32 iValue, nsInt32 iMinValue, nsInt32 iExcludedMaxValue); // [tested]
  ...
  [[nodiscard]] nsInt32 FloorToInt(float f); // [tested]
  ...
  [[nodiscard]] nsInt32 CeilToInt(float f); // [tested]
  ...
  [[nodiscard]] nsInt32 RoundToInt(float f); // [tested]
  ...
  template <typename Type>
  [[nodiscard]] constexpr Type Bitmask_LowN(nsUInt32 uiNumBitsToSet);

  /// \brief Creates a bitmask in which the high N bits are set. For example for N=5, this would be '1111 1000 ... 0000'
  ///
  /// For N >= 32 all bits will be set.
  template <typename Type>
  [[nodiscard]] constexpr Type Bitmask_HighN(nsUInt32 uiNumBitsToSet);
  ...
  [[nodiscard]] nsUInt32 ColorFloatToUnsignedInt(float value);
  ...
  [[nodiscard]] constexpr nsUInt64 MakeUInt64(nsUInt32 uiHigh32, nsUInt32 uiLow32);
