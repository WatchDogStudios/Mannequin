...
nsAndroidApplication::nsAndroidApplication(struct android_app* pApp, nsApplication* pEzApp)
  : m_pApp(pApp)
  , m_pEzApp(pEzApp)
{
  pApp->userData = this;
...
void nsAndroidApplication::HandleCmd(int32_t cmd)
{
  switch (cmd)
  {
    case APP_CMD_INIT_WINDOW:
      ...
      {
        NS_VERIFY(nsRun_Startup(m_pEzApp).Succeeded(), "Failed to startup engine");
        m_bStarted = true;
...
NS_FOUNDATION_DLL void nsAndroidRun(struct android_app* pApp, nsApplication* pEzApp)
{
  nsAndroidApplication androidApp(pApp, pEzApp);

  // This call will loop until APP_CMD_INIT_WINDOW is emitted which triggers nsRun_Startup
  androidApp.AndroidRun();

  nsRun_Shutdown(pEzApp);
