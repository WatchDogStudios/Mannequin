...
thread_local nsJniErrorHandler s_onError;
...
nsJniAttachment::nsJniAttachment()
{
  ...
  s_attachCount++;
  NS_ASSERT_ALWAYS(s_onError.IsValid() == false, "Can't install error handler for more than one instance.");
}
...
{
  s_lastError = state;
  if (s_onError.IsValid() && s_lastError != nsJniErrorState::SUCCESS)
  {
...
void nsJniAttachment::InstallErrorHandler(nsJniErrorHandler onError)
{
  NS_ASSERT_ALWAYS(s_attachCount == 1, "Can't install error handler for more than one instance.");
  s_onError = onError;
}
...
nsJniNullPtr::nsJniNullPtr(nsJniClass& clazz)
{
  m_class = clazz;
}

const nsJniString nsJniNullPtr::GetTypeSignature() const
{
  nsJniString jSignature = m_class.UnsafeCall<nsJniString>("getName", "()Ljava/lang/String;");
  nsStringBuilder signature{jSignature.GetData()};
  signature.ReplaceAll(".", "/");
  return nsJniString{signature.GetData()};
}
