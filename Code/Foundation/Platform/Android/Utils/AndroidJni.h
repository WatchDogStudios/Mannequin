...
struct nsJniError
{
  ...
  NS_ENUM_TO_STRING(SUCCESS, PENDING_EXCEPTION, NO_MATCHING_METHOD, AMBIGUOUS_CALL, NO_MATCHING_FIELD, CALL_ON_NULL_OBJECT, CLASS_NOT_FOUND);
};

using nsJniErrorState = nsEnum<nsJniError>;
using nsJniErrorHandler = nsDelegate<void(nsJniErrorState)>;
...
///   \endcode
///
/// Note that all JNICalls may fail, including the construction of nsJniClass and nsJniObject instances.
...
///       nsJniObject activity = attachment.GetActivity();
///       nsJniClass myClassType = activity.Call<nsJniObject>("getClassLoader").Call<nsJniClass>("loadClass", nsJniString("com.myproject.MyClass"));
///       if(attachment.GetLastError() == nsJniErrorState::NO_MATCHING_METHOD){SpecificErrorHandling();}
///     }
///   \endcode
///
...
///         attachment
///           .GetActivity()
///           .Call<nsJniObject>("getClassLoader")
///           .Call<nsJniClass>("loadClass", nsJniString("com.myproject.MyClass")
///           .CreateInstance()
///           .Call<nsJniObject>("MyMethod");
///       }
...
///   \endcode
///
/// Either way, the error is always internally logged with nsLog::Error.
/// ClearLastError() is called whenever a nsJniAttachment is destroyed, so you needn't worry about cleanup in that case.
...
public:
  ...
  /// \brief Installs an error handler valid for the current nsJniAttachment's lifetime. Called for all nsJniErrorStates but SUCCESS.
  /// There must not be any other nsJniAttachments existing on the same thread for the call to succeed.
  /// There must not be any new nsJniAttachments instances created before the current nsJniAttachment is destroyed.
  ...
  void InstallErrorHandler(nsJniErrorHandler onError);
...
/// \brief Represents the null value of an nsJniClass.
/// Passing null / nullptr directly to nsJni results in type information being lost,
/// without which nsJni can't make the appropriate JNI calls.
/// create an nsJniClass instance instead, and wrap it in an nsJniNullPtr instance.
/// You may then pass that nsJniNullPtr instance to any nsJni calls you make.
class NS_FOUNDATION_DLL nsJniNullPtr
{
  nsJniClass m_class;

public:
  /// \brief Constructs a Class from a nsJniClass.
  ///
  /// \param clazz The nsJniClass.
  explicit nsJniNullPtr(nsJniClass& clazz);

  /// \brief Returns the fully qualified name of the nsJniClass that was passed into the constructor, e.g. "java/lang/String"
  const nsJniString GetTypeSignature() const;
};
