 ...
 */
/// NOTE: This file is for replacing any posix/etc memory functions, and replacing it with the SCE version.
...
extern "C"
{
 ...
 inline int user_malloc_init(void);
  inline int user_malloc_finalize(void);
 inline void* user_malloc(size_t size);
 inline  void* user_calloc(size_t nelem, size_t size);
 inline void* user_realloc(void* ptr, size_t size);
 inline void* user_aligned_alloc(size_t alignment, size_t size);
 inline void* user_memalign(size_t boundary, size_t size);
 inline int user_posix_memalign(void** ptr, size_t boundary, size_t size);
 inline void* user_reallocalign(void* ptr, size_t size, size_t boundary);
 inline int user_malloc_stats(SceLibcMallocManagedSize* mmsize);
 inline int user_malloc_stats_fast(SceLibcMallocManagedSize* mmsize);
...
// J free 関数と置き換わる
inline void user_free(void* ptr)
{
#ifdef ENABLE_ASAN
  if (!ptr)
    return;
...
inline size_t user_malloc_usable_size(void* ptr)
{
#ifdef ENABLE_ASAN
  size_t size;
