...
#define NS_HRESULT_TO_FAILURE_LOG(code)                                                         \
  ...
  {                                                                                             \
    ...
    if (s < 0)                                                                                  \
    {                                                                                           \
      nsLog::Error("Call '{0}' failed with: {1}", NS_PP_STRINGIFY(code), nsHRESULTtoString(s)); \
      return NS_FAILURE;                                                                        \
    }                                                                                           \
  } while (false)
...
#define NS_HRESULT_TO_ASSERT(code)                                                                     \
  ...
  {                                                                                                    \
    nsMinWindows::HRESULT s = (code);                                                                  \
    NS_ASSERT_DEV(s >= 0, "Call '{0}' failed with: {1}", NS_PP_STRINGIFY(code), nsHRESULTtoString(s)); \
  } while (false)
