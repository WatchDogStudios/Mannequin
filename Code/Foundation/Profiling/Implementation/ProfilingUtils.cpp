...
nsResult nsProfilingUtils::SaveProfilingCapture(nsStringView sCapturePath)
{
  ...
  if (fileWriter.Open(sCapturePath) == NS_SUCCESS)
  {
    ...
    // Set sort index to nsInvalidIndex so that the runtime process is always at the bottom and editor is always on top when opening the trace.
    profilingData.m_uiProcessSortIndex = nsInvalidIndex;
