...
///
/// The constructor creates a new scope in the profiling system and the destructor pops
/// the scope. You shouldn't need to use this directly, just use the macro
/// NS_PROFILE_SCOPE provided below.
...
/// \brief This class implements a profiling scope similar to nsProfilingScope, but with
/// additional sub-scopes which can be added easily without introducing actual C++ scopes.
///
/// The constructor pushes one surrounding scope on the stack and then a nested scope as
/// the first section. The function StartNextSection() will end the nested scope and start
/// a new inner scope. This allows to end one scope and start a new one, without having to
/// add actual C++ scopes for starting/stopping profiling scopes.
///
/// You shouldn't need to use this directly, just use the macro NS_PROFILE_LIST_SCOPE
/// provided below.
class nsProfilingListScope
{
public:
  NS_FOUNDATION_DLL nsProfilingListScope(
    nsStringView sListName, nsStringView sFirstSectionName, const char* szFunctionName);
  NS_FOUNDATION_DLL ~nsProfilingListScope();
...
public:
  ...
  static void Capture(
    nsProfilingSystem::ProfilingData& out_capture, bool bClearAfterCapture = false);

  /// \brief Scopes are discarded if their duration is shorter than the specified
  /// threshold. Default is 0.1ms.
  static void SetDiscardThreshold(nsTime threshold);

  using ScopeTimeoutDelegate
    = nsDelegate<void(nsStringView sName, nsStringView sFunctionName, nsTime duration)>;

  /// \brief Sets a callback that is triggered when a profiling scope takes longer than
  /// desired.
  ...
  static void AddCPUScope(nsStringView sName, const char* szFunctionName,
    nsTime beginTime, nsTime endTime, nsTime scopeTimeout);
...
private:
  ...
  ///  Needs to be called before the thread exits to be able to release profiling memory
  ///  of dead threads on Reset.
  static void RemoveThread();

public:
  /// \brief Initialized internal data structures for GPU profiling data. Needs to be
  /// called before adding any data.
  static void InitializeGPUData(nsUInt32 uiGpuCount = 1);

  /// \brief Adds a GPU profiling scope in the internal event ringbuffer.
  static void AddGPUScope(
    nsStringView sName, nsTime beginTime, nsTime endTime, nsUInt32 uiGpuIndex = 0);
};

#if NS_ENABLED(NS_USE_PROFILING) || defined(NS_DOCS)
#  ifndef SUPERLUMINALAPI
/// \brief Profiles the current scope using the given name.
///
/// It is allowed to nest NS_PROFILE_SCOPE, also with NS_PROFILE_LIST_SCOPE. However
/// NS_PROFILE_SCOPE should start and end within the same list scope section.
///
...
#    define NS_PROFILE_SCOPE(ScopeName)                                 \
      nsProfilingScope NS_PP_CONCAT(_nsProfilingScope, NS_SOURCE_LINE)( \
        ScopeName, NS_SOURCE_FUNCTION, nsTime::MakeZero())

/// \brief Same as NS_PROFILE_SCOPE but if the scope takes longer than 'Timeout', the
/// nsProfilingSystem's timeout callback is executed.
///
/// This can be used to log an error or save a callstack, etc. when a scope exceeds an
/// expected amount of time.
///
/// \sa nsProfilingSystem::SetScopeTimeoutCallback()
#    define NS_PROFILE_SCOPE_WITH_TIMEOUT(ScopeName, Timeout)           \
      nsProfilingScope NS_PP_CONCAT(_nsProfilingScope, NS_SOURCE_LINE)( \
        ScopeName, NS_SOURCE_FUNCTION, Timeout)

/// \brief Profiles the current scope using the given name as the overall list scope name
/// and the section name for the first section in the list.
///
...
#    define NS_PROFILE_LIST_SCOPE(ListName, FirstSectionName)               \
      nsProfilingListScope NS_PP_CONCAT(_nsProfilingScope, NS_SOURCE_LINE)( \
        ListName, FirstSectionName, NS_SOURCE_FUNCTION)
...
#  endif
#else
/// NOTE(Mikael A.): Profiling is disabled in production builds!!!
/// \brief Profiles the current scope using the given name.
///
/// It is allowed to nest NS_PROFILE_SCOPE, also with NS_PROFILE_LIST_SCOPE. However
/// NS_PROFILE_SCOPE should start and end within the same list scope section.
...
#include <Foundation/Profiling/Profiling_Superluminal.h>
