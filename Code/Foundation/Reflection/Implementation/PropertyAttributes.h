...
private:
  ...
  // Not pretty, but the values in the array are either created using 'new' when using this class as a reflection decoration, or created using 'NS_DEFAULT_NEW' when serialized and sent to the editor so in the dtor we need to know where these came from.
...
/// For example if "nsComponent" is passed in, only types derived from nsComponent are listed.
class NS_FOUNDATION_DLL nsRttiTypeStringAttribute : public nsTypeWidgetAttribute
{
  NS_ADD_DYNAMIC_REFLECTION(nsRttiTypeStringAttribute, nsTypeWidgetAttribute);

public:
  nsRttiTypeStringAttribute() = default;
  nsRttiTypeStringAttribute(const char* szBaseType)
