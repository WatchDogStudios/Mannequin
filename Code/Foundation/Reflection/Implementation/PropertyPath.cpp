...
nsResult nsPropertyPath::WriteToLeafObject(void* pRootObject, const nsRTTI* pType, nsDelegate<void(void* pLeaf, const nsRTTI& pType)> func) const
{
  NS_ASSERT_DEBUG(
...
nsResult nsPropertyPath::ReadFromLeafObject(void* pRootObject, const nsRTTI* pType, nsDelegate<void(void* pLeaf, const nsRTTI& pType)> func) const
{
  NS_ASSERT_DEBUG(
    m_PathSteps.IsEmpty() || m_PathSteps[m_PathSteps.GetCount() - 1].m_pProperty->GetSpecificType()->GetTypeFlags().IsSet(nsTypeFlags::Class),
    "To resolve the leaf object the path needs to be empty or end in a class.");
