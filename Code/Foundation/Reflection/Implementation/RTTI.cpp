...
nsRTTI::~nsRTTI()
{
  ...
  // To ensure unloading plugins does not leak any heap allocated attributes etc, we need to properly clean up the RTTI members. The assumption is that anything that is deleted here was created using global 'new' when declaring the reflection information inside NS_BEGIN_PROPERTIES etc. Thus, any derived nsRTTI class must make sure these arrays are cleared out before this destructor is called.
  if (m_sPluginName != "Static")
  {
    ...
    {
      auto pPropNonConst = const_cast<nsAbstractProperty*>(pProp);
      delete pPropNonConst;
    }
    for (auto pFunc : m_Functions)
    {
      auto pFuncNonConst = const_cast<nsAbstractFunctionProperty*>(pFunc);
