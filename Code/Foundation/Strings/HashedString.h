...
public:
  ...
  NS_ALWAYS_INLINE bool operator==(const char* szString) const { return GetString().GetView() == nsStringView(szString); }
  NS_ADD_DEFAULT_OPERATOR_NOTEQUAL(const char*);
  ...
  /// The string hash may not be known, if the value was never assigned to any nsHashedString, in which case NS_FAILURE is returned.
  static nsResult LookupStringHash(nsUInt64 uiHash, nsStringView& out_sResult);
...
// since we allow to cast implicitly to const char*, we need these overloads to not do a pure pointer comparison
NS_ALWAYS_INLINE bool operator==(const char* szString, const nsHashedString& rhs)
{
  return rhs.GetView() == nsStringView(szString);
}
...
public:
  ...
  /// \brief Convenience function to call nsHashedString::LookupStringHash().
  nsResult LookupStringHash(nsStringView& out_sResult) const
  {
    return nsHashedString::LookupStringHash(m_uiHash, out_sResult);
  }
