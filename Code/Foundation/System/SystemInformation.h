...
/// \brief The system configuration class encapsulates information about the system the
/// application is running on.
///
/// Retrieve the system configuration by using nsSystemInformation::Get(). If you use the
/// system configuration in startup code make sure to add the correct dependency to the
/// system "SystemInformation" in "Foundation".
...
public:
  ...
  nsUInt32 GetAllowedCPUCoreCount() const { return m_uiCPUCoreAllowedCount; }
  ...
  /// \brief Returns a struct that contains detailed information about the available CPU
  /// features (SIMD support).
...
private:
  ...
  nsUInt32 m_uiCPUCoreAllowedCount;
