...
template <typename BlockStorageAllocator /*= nsDefaultAllocatorWrapper*/>
bool nsTagSetTemplate<BlockStorageAllocator>::IsSetByName(const nsTempHashedString& sTag) const
{
  if (const nsTag* tag = nsTagRegistry::GetGlobalRegistry().GetTagByName(sTag))
  {
    return IsSet(*tag);
  }
