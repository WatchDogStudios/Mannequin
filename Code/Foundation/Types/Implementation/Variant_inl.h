...
NS_FORCE_INLINE void nsVariant::InitInplace(const T& value)
{
  ...
  static_assert(nsGetTypeClass<T>::value <= nsTypeIsMemRelocatable::value, "in place data needs to be POD or mem relocatable");
...
NS_FORCE_INLINE void nsVariant::InitTypedObject(const T& value, nsTraitInt<1>)
{
  ...
  static_assert(TypeDeduction<T>::value == Type::TypedObject, "value of this type cannot be stored in a Variant");
  static_assert(nsIsPodType<T>::value, "in place data needs to be POD");
  nsMemoryUtils::CopyConstruct(reinterpret_cast<T*>(&m_Data), value, 1);
