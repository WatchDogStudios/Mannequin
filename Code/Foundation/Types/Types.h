...
static_assert(sizeof(nsInt8) == 1);
static_assert(sizeof(nsInt16) == 2);
static_assert(sizeof(nsInt32) == 4);
static_assert(sizeof(nsInt64) == 8); // must be defined in the specific compiler header
static_assert(sizeof(nsUInt8) == 1);
static_assert(sizeof(nsUInt16) == 2);
static_assert(sizeof(nsUInt32) == 4);
static_assert(sizeof(nsUInt64) == 8); // must be defined in the specific compiler header
...
static_assert(sizeof(void*) == NS_ALIGNMENT_MINIMUM);
static_assert(alignof(void*) == NS_ALIGNMENT_MINIMUM);
...
#define NS_SUCCEED_OR_RETURN_LOG(code)                                       \
  ...
  {                                                                          \
    ...
    if (nsToResult(s).Failed())                                              \
    {                                                                        \
      nsLog::Error("Call '{0}' failed with: {1}", NS_PP_STRINGIFY(code), s); \
      return s;                                                              \
    }                                                                        \
  } while (false)
...
#define NS_SUCCEED_OR_RETURN_CUSTOM_LOG(code, log)                             \
  ...
  {                                                                            \
    ...
    if (nsToResult(s).Failed())                                                \
    {                                                                          \
      nsLog::Error("Call '{0}' failed with: {1}", NS_PP_STRINGIFY(code), log); \
      return s;                                                                \
    }                                                                          \
  } while (false)
