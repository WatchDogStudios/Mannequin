...
public:
  ...
  NS_ALWAYS_INLINE nsUuid() = default; // [tested]

  /// \brief Constructs the Uuid from existing values
  NS_ALWAYS_INLINE constexpr nsUuid(nsUInt64 uiLow, nsUInt64 uiHigh)
    : m_uiHigh(uiHigh)
    , m_uiLow(uiLow)
  {
  }
  ...
  /// \brief Returns a human-readable string representation of the Uuid.
  [[nodiscard]] NS_ALWAYS_INLINE const char* ToString()
  {
    static char s_szBuffer[37];
    snprintf(s_szBuffer, sizeof(s_szBuffer), "%016llx-%016llx", m_uiHigh, m_uiLow);
    return s_szBuffer;
  }
