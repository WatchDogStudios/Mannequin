...
{
  NS_DECLARE_POD_TYPE();
  nsVarianceTypeFloat() = default;
  nsVarianceTypeFloat(float value, float fVariance = 0.0f)
    : m_Value(value)
  {
...
{
  NS_DECLARE_POD_TYPE();
  nsVarianceTypeTime() = default;
  nsVarianceTypeTime(nsTime value, float fVariance = 0.0f)
    : m_Value(value)
  {
    m_fVariance = fVariance;
  }
...
{
  NS_DECLARE_POD_TYPE();
  nsVarianceTypeAngle() = default;
  nsVarianceTypeAngle(nsAngle value, float fVariance = 0.0f)
    : m_Value(value)
  {
    m_fVariance = fVariance;
  }
  // Alternate ordering to support accidental brace-init patterns in legacy tests
  nsVarianceTypeAngle(float fVariance, nsAngle value)
    : m_Value(value)
  {
    m_fVariance = fVariance;
  }
