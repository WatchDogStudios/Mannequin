...
#define Check(name)                                     \
  if (sColorName.IsEqual_NoCase(NS_PP_STRINGIFY(name))) \
  return nsColor::name

  nsResult ConvertHexStringToColor(nsStringView sText, nsColorGammaUB& ref_color)
  {
    ref_color = nsColorGammaUB(0, 0, 0);

    auto twoCharsToByte = [](nsStringView& text, nsUInt8& out_uiByte) -> nsResult
    {
      if (text.IsEmpty())
        return NS_SUCCESS;
    ...
    NS_SUCCEED_OR_RETURN(twoCharsToByte(sText, ref_color.r));
    NS_SUCCEED_OR_RETURN(twoCharsToByte(sText, ref_color.g));
    NS_SUCCEED_OR_RETURN(twoCharsToByte(sText, ref_color.b));
    NS_SUCCEED_OR_RETURN(twoCharsToByte(sText, ref_color.a));
