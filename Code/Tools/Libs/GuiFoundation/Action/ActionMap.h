...
public:
  ...
  nsActionMap(nsStringView sParentMapping);
  ~nsActionMap();
  ...
  void HideAction(nsActionDescriptorHandle hAction, nsStringView sPath);
...
private:
  ...
  {
    nsActionDescriptorHandle m_hAction;
    nsString m_sPath;
    nsString m_sSubPath;
    float m_fOrder;
  };
  ...
  void MapActionInternal(nsActionDescriptorHandle hAction, nsStringView sPath, float fOrder);
  void MapActionInternal(nsActionDescriptorHandle hAction, nsStringView sPath, nsStringView sSubPath, float fOrder);
  nsResult UnmapActionInternal(nsActionDescriptorHandle hAction, nsStringView sPath);

  nsUuid MapActionInternal(const nsActionMapDescriptor& desc);
  nsResult UnmapActionInternal(const nsActionMapDescriptor& desc);
  nsResult UnmapActionInternal(const nsUuid& guid);
...
private:
  nsString m_sParentMapping;
  nsDynamicArray<TempActionMapDescriptor> m_TempActions;
  nsDynamicArray<TempActionMapDescriptor> m_TempHiddenActions;
  nsUInt32 m_uiEditCounter = 0;

  mutable nsUInt32 m_uiTransitiveEditCounterOfRoot = 0;
  mutable TreeNode m_Root;
  mutable nsMap<nsUuid, nsTreeNode<nsActionMapDescriptor>*> m_Descriptors;
};
