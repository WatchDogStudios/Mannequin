...
{
  static const void MakeValidMaterialName(nsString& ref_sTarget, const char* szSource, nsUInt32 uiMatIdx, nsSet<nsString>& ref_knownMaterialNames)
  {
    nsStringBuilder tmp;
    nsPathUtils::MakeValidFilename(szSource, '_', tmp);
  ...
  static void TryReadAssimpProperty(nsMap<PropertySemantic, nsVariant>& inout_properties, PropertySemantic targetSemantic, const aiMaterial& assimpMaterial, const char* szKey, nsUInt32 uiType, nsUInt32 uiIdx, bool bInvert = false)
  {
    assimpType value;
  ...
  void TryReadAssimpTextures(nsMap<TextureSemantic, nsString>& out_textures, aiTextureType type, TextureSemantic targetSemantic, const aiMaterial& assimpMaterial)
  {
  ...
  nsResult ImporterAssimp::ImportMaterials()
  {
    if (!m_pScene->HasMaterials())
      return NS_SUCCESS;

    nsSet<nsString> knownMaterialNames;
    knownMaterialNames.Insert("");

    for (nsUInt32 matIdx = 0; matIdx < m_pScene->mNumMaterials; ++matIdx)
    {
