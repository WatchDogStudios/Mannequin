...
nsCommandLineOptionEnum opt_Mode("_TexConv", "-mode", "Mode determines which arguments need to be set.\n\
...
nsCommandLineOptionPath opt_Out("_TexConv", "-out",
  "Absolute path to main output file.\n\
   ext = tga, dds, nsBinTexture2D, nsBinTexture3D, nsBinTextureCube or nsBinTextureAtlas.",
  "");
...
nsCommandLineOptionString opt_CompareHtmlTitle("_TexConv", "-cmpHtml", "Title for the compare result HTML. If empty no HTML file is written.", "");
nsCommandLineOptionPath opt_CompareActual("_TexConv", "-cmpImg", "Path to an image to compare with another.", "");
nsCommandLineOptionPath opt_CompareExpected("_TexConv", "-cmpRef", "Path to a reference image to compare against.", "");
nsCommandLineOptionInt opt_CompareThreshold("_TexConv", "-cmpMSE", "The error threshold for the comparison to be considered as failed.\n\
  No output files are written, if the image difference is below this value.",
  100, 0);
nsCommandLineOptionBool opt_CompareRelaxed("_TexConv", "-cmpRelaxed", "Use a more lenient comparison method.\nUseful for images with single-pixel wide rasterized lines.", false);
...
nsResult nsTexConv::ParseCommandLine()
{
  ...
  NS_SUCCEED_OR_RETURN(ParseMode());

  if (m_Mode == nsTexConvMode::Compare)
  {
    NS_SUCCEED_OR_RETURN(ParseCompareMode());
  }
  else
  {
...
nsResult nsTexConv::ParseMode()
{
  switch (opt_Mode.GetOptionValue(nsCommandLineOption::LogMode::FirstTime))
  {
    case 0:
      m_Mode = nsTexConvMode::Convert;
      return NS_SUCCESS;

    case 1:
      m_Mode = nsTexConvMode::Compare;
      return NS_SUCCESS;
  }

  nsLog::Error("Invalid mode selected.");
  return NS_FAILURE;
}

nsResult nsTexConv::ParseCompareMode()
{
  m_sOutputFile = opt_Out.GetOptionValue(nsCommandLineOption::LogMode::Always);

  if (m_sOutputFile.IsEmpty())
  {
    nsLog::Warning("Output path is not specified. Use option '-out \"path\"' to set the prefix path for the output files.");
  }

  m_sHtmlTitle = opt_CompareHtmlTitle.GetOptionValue(nsCommandLineOption::LogMode::FirstTime);

  nsStringBuilder tmp, res;
  const auto pCmd = nsCommandLineUtils::GetGlobalInstance();

  m_Comparer.m_Descriptor.m_sActualFile = opt_CompareActual.GetOptionValue(nsCommandLineOption::LogMode::FirstTime);
  m_Comparer.m_Descriptor.m_sExpectedFile = opt_CompareExpected.GetOptionValue(nsCommandLineOption::LogMode::FirstTime);
  m_Comparer.m_Descriptor.m_MeanSquareErrorThreshold = opt_CompareThreshold.GetOptionValue(nsCommandLineOption::LogMode::FirstTime);
  m_Comparer.m_Descriptor.m_bRelaxedComparison = opt_CompareRelaxed.GetOptionValue(nsCommandLineOption::LogMode::FirstTime);

  if (m_Comparer.m_Descriptor.m_sActualFile.IsEmpty())
  {
    nsLog::Error("Image to compare is not specified.");
    return NS_FAILURE;
  }

  if (m_Comparer.m_Descriptor.m_sExpectedFile.IsEmpty())
  {
    nsLog::Error("Reference image to compare against is not specified.");
    return NS_FAILURE;
  }
