...
NS_CREATE_SIMPLE_TEST(CodeUtils, Preprocessor)
{
  ...
  NS_TEST_BOOL(nsFileSystem::DetectSdkRootDirectory() == NS_SUCCESS);
  NS_TEST_BOOL(nsFileSystem::AddDataDirectory(sReadDir, "PreprocessorTest") == NS_SUCCESS);
  NS_TEST_BOOL_MSG(nsFileSystem::AddDataDirectory(sWriteDir, "PreprocessorTest", "output", nsDataDirUsage::AllowWrites) == NS_SUCCESS, "Failed to mount data dir '%s'", sWriteDir.GetData());
