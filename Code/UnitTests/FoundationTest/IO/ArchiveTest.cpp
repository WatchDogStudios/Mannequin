...
NS_CREATE_SIMPLE_TEST(IO, Archive)
{
  ...
  if (!NS_TEST_BOOL(nsFileSystem::AddDataDirectory(sOutputFolder, "Clear", "output", nsDataDirUsage::AllowWrites).Succeeded()))
    return;
...
#  endif
  ...
  {
    if (!NS_TEST_BOOL(nsFileSystem::AddDataDirectory(sArchiveFile, "Clear", "archive", nsDataDirUsage::ReadOnly) == NS_SUCCESS))
      return;
    ...
    // mount a second time
    if (!NS_TEST_BOOL(nsFileSystem::AddDataDirectory(sArchiveFile, "Clear", "archive2", nsDataDirUsage::ReadOnly) == NS_SUCCESS))
      return;
  }
