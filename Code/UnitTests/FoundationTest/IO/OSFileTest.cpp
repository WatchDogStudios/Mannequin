...
#  endif // NS_SUPPORTS_CASE_INSENSITIVE_PATHS && NS_SUPPORTS_UNRESTRICTED_FILE_ACCESS
  ...
  {
    nsStringBuilder sOutputFolder = nsFileSystem::GetSdkRootDirectory();
...
#  endif
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "File Iterator (non recursive)")
  {
    nsStringBuilder sOutputFolder = nsFileSystem::GetSdkRootDirectory();
    sOutputFolder.AppendPath("Data/Base");
    ...
    nsFileSystemIterator it;
    for (it.StartSearch(sOutputFolder.GetData(), nsFileSystemIteratorFlags::ReportFiles); it.IsValid();)
    {
      sFullPath = it.GetCurrentPath();
      sFullPath.AppendPath(it.GetStats().m_sName.GetData());
      ...
      NS_TEST_BOOL(sFullPath.StartsWith(sOutputFolder));
      sFullPath.MakeRelativeTo(sOutputFolder).AssertSuccess();

      NS_TEST_BOOL(!sFullPath.FindSubString("/")); // no sub path
