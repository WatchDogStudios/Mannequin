...
NS_CREATE_SIMPLE_TEST(Math, Intersection)
{
    ...
    for (nsUInt32 i = 0; i < 100; ++i)
    {
      ...
      for (float y = -14.5; y <= 14.5f; y += 2.0f)
      {
        for (float x = -14.5; x <= 14.5f; x += 2.0f)
        {
          ...
          NS_TEST_BOOL(nsIntersectionUtils::RayPolygonIntersection(vRayStart, vRayDir, Vertices, 4, &fIntersection, &vIntersection, sizeof(nsVec3) * 2) == bIntersects);
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "RayTriangleIntersection")
  {
    for (nsUInt32 i = 0; i < 100; ++i)
    {
      nsMat4 m;
      ...
      nsVec3 Vertices[4] = {m.TransformPosition(nsVec3(-10, -10, 0)), m.TransformPosition(nsVec3(10, -10, 0)),
        m.TransformPosition(nsVec3(10, 10, 0)), m.TransformPosition(nsVec3(-10, 10, 0))};
      ...
      for (float y = -14.5; y <= 14.5f; y += 2.0f)
      {
        for (float x = -14.5; x <= 14.5f; x += 2.0f)
        {
          ...
          nsVec3 vPlaneIntersection;
          nsPlane plane = nsPlane::MakeFromPoints(Vertices[0], Vertices[1], Vertices[2]);
          NS_TEST_BOOL(plane.GetRayIntersection(vRayStart, vRayDir, nullptr, &vPlaneIntersection));
          const bool bIsOnEdge = nsIntersectionUtils::IsPointOnLine(Vertices[0], Vertices[2], vPlaneIntersection);

          float fIntersection1 = 0;
          nsVec3 vIntersection1(0.0f);
          float fIntersection2 = 0;
          nsVec3 vIntersection2(0.0f);

          const bool bHit1a = nsIntersectionUtils::RayPolygonIntersection(vRayStart, vRayDir, Vertices, 3);

          const bool bHit1 = nsIntersectionUtils::RayTriangleIntersection(vRayStart, vRayDir, Vertices[0], Vertices[1], Vertices[2], &fIntersection1, &vIntersection1);

          NS_TEST_BOOL(bHit1 == bHit1a);

          const bool bHit2 = nsIntersectionUtils::RayTriangleIntersection(vRayStart, vRayDir, Vertices[0], Vertices[2], Vertices[3], &fIntersection2, &vIntersection2);

          if (!bInRect)
          {
            // outside rect, neither should hit
            NS_TEST_BOOL(bHit1 == false);
            NS_TEST_BOOL(bHit2 == false);
            NS_TEST_BOOL(bHit1a == false);
          }
          else
          {
            if (!bIsOnEdge) // anything can happen close to the edge
            {
              ...
              NS_TEST_BOOL(bHit1 || bHit2);
              NS_TEST_BOOL(bHit1 != bHit2);
            }

            if (bHit1)
            {
              NS_TEST_FLOAT(fIntersection1, 3.0f, 0.0001f);
              NS_TEST_VEC3(vIntersection1, m.TransformPosition(nsVec3(x, y, 0.0f)), 0.0001f);
            }

            if (bHit2)
            {
              NS_TEST_FLOAT(fIntersection2, 3.0f, 0.0001f);
              NS_TEST_VEC3(vIntersection2, m.TransformPosition(nsVec3(x, y, 0.0f)), 0.0001f);
            }
          }
        }
      }
