    ...
    nsRTTI::ForEachType(
      [&](const nsRTTI* pRtti)
      {
        if (pRtti->GetTypeName() == "nsTestStruct2")
        {
          ...
          {
            NS_ANALYSIS_ASSUME(pProp != nullptr);
...
NS_CREATE_SIMPLE_TEST(Reflection, Hierarchies)
{
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "nsTestClass1")
  {
    ...
    if (NS_TEST_BOOL(pInstance != nullptr))
    {
      NS_ANALYSIS_ASSUME(pInstance != nullptr);
...
NS_CREATE_SIMPLE_TEST(Reflection, Pointer)
{
  ...
  if (!NS_TEST_BOOL(pRtti != nullptr))
    return;
  NS_ANALYSIS_ASSUME(pRtti != nullptr);
