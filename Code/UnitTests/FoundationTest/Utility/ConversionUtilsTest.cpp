...
#define Check(name)                                                                     \
  {                                                                                     \
    bool valid = false;                                                                 \
    const nsColor c = nsConversionUtils::GetColorByName(NS_PP_STRINGIFY(name), &valid); \
    NS_TEST_BOOL(valid);                                                                \
    nsString sName = nsConversionUtils::GetColorName(c);                                \
    NS_TEST_STRING(sName, NS_PP_STRINGIFY(name));                                       \
  }
...
#define Check2(name, otherName)                                                         \
  {                                                                                     \
    ...
    NS_TEST_BOOL(valid);                                                                \
    nsString sName = nsConversionUtils::GetColorName(c);                                \
    NS_TEST_STRING(sName, NS_PP_STRINGIFY(otherName));                                  \
  }
