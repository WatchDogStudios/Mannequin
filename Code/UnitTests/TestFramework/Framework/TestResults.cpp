...
bool nsTestFrameworkResult::WriteJsonToFile(const char* szFileName) const
{
...

  {
    ...
    if (nsPathUtils::IsAbsolutePath(szFileName))
    {
      ...
      if (nsFileSystem::AddDataDirectory("", "jsonoutput", ":", nsDataDirUsage::AllowWrites).Failed())
        return false;
    ...
    else
    {
      ...
      if (nsFileSystem::AddDataDirectory(">nstest/", "jsonoutput", ":", nsDataDirUsage::AllowWrites).Failed())
        return false;
