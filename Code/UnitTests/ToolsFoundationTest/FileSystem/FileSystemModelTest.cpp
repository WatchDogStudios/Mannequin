...
void FileSystemModelTest()
{
  ...
  auto CompareFiles = [&](nsArrayPtr<nsFileChangedEvent> expected)
  {
    ...
    {
      for (nsUInt32 i = 0; i < expected.GetCount(); i++)
      {
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Startup")
  {
    ...
    // for absolute paths
    NS_TEST_BOOL(nsFileSystem::AddDataDirectory("", "", ":", nsDataDirUsage::AllowWrites) == NS_SUCCESS);
    NS_TEST_BOOL(nsFileSystem::AddDataDirectory(sOutputFolder, "Clear", "output", nsDataDirUsage::AllowWrites) == NS_SUCCESS);
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "git")
  {
    nsStringBuilder sIndex(sOutputFolder);
    sIndex.AppendPath("index");
    nsStringBuilder sLock(sOutputFolder);
    sLock.AppendPath("index.lock");

    NS_TEST_RESULT(nstCreateFile(sIndex));
    ...
    {
      nsFileChangedEvent expected[] = {nsFileChangedEvent(MakePath(sIndex), {}, nsFileChangedEvent::Type::FileAdded)};
      CompareFiles(nsMakeArrayPtr(expected));
      ClearFiles();
    }
...
#  endif
    ...
    NS_TEST_RESULT(nstCreateFile(sLock));
    ...
    {
      nsFileChangedEvent expected[] = {nsFileChangedEvent(MakePath(sLock), {}, nsFileChangedEvent::Type::FileAdded)};
      CompareFiles(nsMakeArrayPtr(expected));
      ClearFiles();
    }

    NS_TEST_RESULT(nsOSFile::DeleteFile(sIndex));
    NS_TEST_RESULT(nsOSFile::MoveFileOrDirectory(sLock, sIndex));
    ...
    nsFileChangedEvent expected[] = {
      nsFileChangedEvent(MakePath(sIndex), {}, nsFileChangedEvent::Type::FileChanged),
      nsFileChangedEvent(MakePath(sLock), {}, nsFileChangedEvent::Type::FileRemoved)};
    CompareFiles(nsMakeArrayPtr(expected));
    ClearFiles();
    CompareFolders({});
...
NS_CREATE_SIMPLE_TEST(FileSystem, FileSystemModelNonNTFS)
{
  auto* pForceNonNTFS = static_cast<nsCVarBool*>(nsCVar::FindCVarByName("DirectoryWatcher.ForceNonNTFS"));
  *pForceNonNTFS = true;
  FileSystemModelTest();
  *pForceNonNTFS = false;
}
